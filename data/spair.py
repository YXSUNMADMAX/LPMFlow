...
class SPairDataset(CorrespondenceDataset):
    ...
    def __init__(self, benchmark, datapath, thres, device, split, augmentation, feature_size):
        ...
        self.pair_ids = list(map(lambda x: x.split('-')[0], self.train_data))
        ...
        self.src_size = list(map(lambda x: torch.tensor(x["src_imsize"]).float(), anntn_files))
        self.trg_size = list(map(lambda x: torch.tensor(x["trg_imsize"]).float(), anntn_files))
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'trn' and self.augmentation:
            ...
            b = random_crop(batch['trg_img'], batch['trg_kps'], self.trg_bbox[idx].clone(),
                            org_size=self.trg_size[idx], size=(self.imside,) * 2)
            batch['trg_img'], batch['trg_kps'] = b

            a = random_crop(batch['src_img'], batch['src_kps'], self.src_bbox[idx].clone(),
                        org_size=self.src_size[idx], size=(self.imside,) * 2)
            batch['src_img'], batch['src_kps'] = a
        ...
        batch['flow_src'] = self.kps_to_flow(batch)
        batch['flow_trg'] = self.kps_to_flow_RE(batch)
