...
class PFPascalDataset(CorrespondenceDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'trn' and self.augmentation:
            batch['src_img'], batch['src_kps'] = random_crop(batch['src_img'], batch['src_kps'], self.src_bbox[idx].clone().int(), org_size=batch['src_imsize'], size=(self.imside,)*2)
            batch['trg_img'], batch['trg_kps'] = random_crop(batch['trg_img'], batch['trg_kps'], self.trg_bbox[idx].clone().int(), org_size=batch['trg_imsize'], size=(self.imside,)*2)
