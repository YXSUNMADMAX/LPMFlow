...
class KeypointToFlow:
    def __init__(self, receptive_field_size=35, jsz=16, feat_size=16, img_size=384):
    ...
    def __call__(self, batch):
        ...
        kp_src = trg_kps.narrow_copy(0, 0, n_pts)
        kp = src_kps.narrow_copy(0, 0, n_pts)
...
class KeypointToFlow_RE:
    ...
    def __call__(self, batch):
        src_kps, trg_kps, n_pts = batch['trg_kps'].t(), batch['src_kps'].t(), batch['n_pts']
...
class TPSFlow:
    ...
    def __call__(self, batch):
        src_kps, trg_kps, n_pts = batch['src_per_kps'].t(), batch['trg_per_kps'].t(), batch['n_per_pts']
...
class TPSFlow_RE:
    ...
    def __call__(self, batch):
        src_kps, trg_kps, n_pts = batch['trg_per_kps'].t(), batch['src_per_kps'].t(), batch['n_per_pts']
