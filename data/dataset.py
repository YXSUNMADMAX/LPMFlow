...
from .keypoint_to_flow import KeypointToFlow, TPSFlow, KeypointToFlow_RE, TPSFlow_RE
...
def random_crop(img, kps, bbox_t, org_size=(512, 512), size=(512, 512), p=0.7):
    ...
    # print(bbox)
    # print(w0,h0)
    bbox = bbox_t.clone().numpy()
    bbox[0] = bbox[0] * 512 // h0
    bbox[1] = bbox[1] * 512 // w0
    bbox[2] = bbox[2] * 512 // h0
    bbox[3] = bbox[3] * 512 // w0
    kps = kps.t()
    ...
    def __init__(self, benchmark, datapath, thres, device, split, augmentation, feature_size):
        ...
        if split == 'trn' and augmentation:
            self.transform = A.Compose([
                A.Resize(self.imside, self.imside),
                A.ToGray(p=0.2),
                A.Posterize(p=0.2),
                A.Equalize(p=0.2),
        ...
        self.kps_to_flow_RE = KeypointToFlow_RE(receptive_field_size=35, jsz=512 // feature_size, feat_size=feature_size,
                                          img_size=self.imside)
