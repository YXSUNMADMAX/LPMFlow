...
from models.LPMFlow import LPMFlow
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scheduler', type=str, default='step', choices=['step', 'cosine'])#default='step'
    ...
    parser.add_argument('--freeze', type=boolean_string, nargs='?', const=False, default=False)
    parser.add_argument('--augmentation', type=boolean_string, nargs='?', const=True, default=True)
    parser.add_argument('--ibot_ckp_file', type=str, default='./checkpoint.pth')  ### donot forget this argument for iBOT ckp path!!!!
    ...
    model = LPMFlow(
        feature_size=args.feature_size, ibot_ckp_file=args.ibot_ckp_file,
    ...
    for name, param in model.named_parameters():
        ...
        elif 'feature_extraction' in name or 'mask_extraction' in name:
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs, eta_min=1e-6,
                                               verbose=True) if args.scheduler == "cosine" else lr_scheduler.MultiStepLR(
